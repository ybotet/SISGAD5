...
interface GruposTrabajoPaginationProps {
...
export default function GruposTrabajoPagination({
    ...
    onNext,
    onPrev
}: GruposTrabajoPaginationProps) {
    return (
        <div className="bg-white rounded-lg shadow p-4 mt-4">
            <div className="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
                <div className="text-sm text-gray-600">
                    Mostrando {pagination.page * pagination.limit - pagination.limit + 1} -{' '}
                    {Math.min(pagination.page * pagination.limit, pagination.total)} de {pagination.total} grupos
                </div>
